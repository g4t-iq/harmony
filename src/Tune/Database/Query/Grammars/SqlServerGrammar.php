...
namespace Tune\Database\Query\Grammars;

use Tune\Database\Query\Builder;
...
class SqlServerGrammar extends Grammar
{
     ...
     *
     * @param  \Tune\Database\Query\Builder  $query
     * @param  \Tune\Database\Query\IndexHint  $indexHint
     * @return string
     */
    ...
    public function compileUpsert(Builder $query, array $values, array $uniqueBy, array $update)
    {
        ...
        $sql .= 'using (values '.$parameters.') '.$this->wrapTable('harmony_source').' ('.$columns.') ';

        $on = collect($uniqueBy)->map(function ($column) use ($query) {
            return $this->wrap('harmony_source.'.$column).' = '.$this->wrap($query->from.'.'.$column);
        })->implode(' and ');
        ...
        if ($update) {
            $update = collect($update)->map(function ($value, $key) {
                return is_numeric($key)
                    ? $this->wrap($value).' = '.$this->wrap('harmony_source.'.$value)
