...
namespace Tune\Database\Console\Migrations;

use Tune\Console\ConfirmableTrait;
use Tune\Contracts\Console\Isolatable;
use Tune\Contracts\Events\Dispatcher;
use Tune\Database\Events\SchemaLoaded;
use Tune\Database\Migrations\Migrator;
use Tune\Database\SQLiteDatabaseDoesNotExistException;
use Tune\Database\SqlServerConnection;
use PDOException;
use Throwable;

use function Harmony\Prompts\confirm;
...
class MigrateCommand extends BaseCommand implements Isolatable
{
     ...
     *
     * @var \Tune\Database\Migrations\Migrator
     */
    protected $migrator;
     ...
     *
     * @param  \Tune\Database\Migrations\Migrator  $migrator
    ...
    {
        if ($this->harmony['config']->get("database.connections.{$connection->getName()}.database") !== $connection->getDatabaseName()) {
            return false;
        }
        ...
        try {
            $this->harmony['config']->set("database.connections.{$connection->getName()}.database", null);

            $this->harmony['db']->purge();
            ...
            $this->harmony['config']->set("database.connections.{$connection->getName()}.database", $connection->getDatabaseName());
        }
    }
