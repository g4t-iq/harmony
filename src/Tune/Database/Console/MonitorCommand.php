...
use Tune\Database\Events\DatabaseBusy;
...
class MonitorCommand extends DatabaseInspectionCommand
{
     ...
     *
     * @param  \Tune\Database\ConnectionResolverInterface  $connection
    ...
    protected function parseDatabases($databases)
    {
        return collect(explode(',', $databases))->map(function ($database) {
            if (! $database) {
                $database = $this->harmony['config']['database.default'];
            }
     ...
     *
     * @param  \Tune\Support\Collection  $databases
     * @return void
     */
