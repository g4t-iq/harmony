...
use Tune\Http\Client\Factory as Http;
...
use function Harmony\Prompts\suggest;
...
class DocsCommand extends Command
{
     ...
     * @var string
     */
    protected $description = 'Access the Harmony documentation';
     ...
     *
     * @param  \Tune\Http\Client\Factory  $http
    ...
    {
        if ($this->isSearching()) {
            return "https://harmony.com/docs/{$this->version()}?".Arr::query([
                'q' => $this->searchQuery(),
            ]);
        }

        return with($this->page(), function ($page) {
            return trim("https://harmony.com/docs/{$this->version()}/{$page}#{$this->section($page)}", '#/');
        });
    }
    ...
    /**
     * Fetch the documentation index from the Harmony website.
     *
     * @return \Tune\Http\Client\Response
     */
    protected function fetchDocs()
    {
        return $this->http->get("https://harmony.com/docs/{$this->version()}/index.json");
    }
    ...
    {
        return Str::before($this->version ?? $this->harmony->version(), '.').'.x';
    }
