...
use Tune\Routing\ViewController;
...
class RouteListCommand extends Command
{
    ...
    protected function isFrameworkController(Route $route)
    {
        return in_array($route->getControllerClass(), [
            '\Tune\Routing\RedirectController',
            '\Tune\Routing\ViewController',
        ], true);
    }
     ...
     *
     * @param  \Tune\Support\Collection  $routes
     * @return string
     */
    ...
    protected function formatActionForCli($route)
    {
        ...
        $rootControllerNamespace = $this->harmony[UrlGenerator::class]->getRootControllerNamespace()
            ?? ($this->harmony->getNamespace().'Http\\Controllers');
