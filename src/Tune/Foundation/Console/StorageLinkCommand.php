...
class StorageLinkCommand extends Command
{
    ...
    public function handle()
    {
        ...
        foreach ($this->links() as $link => $target) {
            ...
            if (is_link($link)) {
                $this->harmony->make('files')->delete($link);
            }

            if ($relative) {
                $this->harmony->make('files')->relativeLink($target, $link);
            } else {
                $this->harmony->make('files')->link($target, $link);
            }
    ...
    {
        return $this->harmony['config']['filesystems.links'] ??
