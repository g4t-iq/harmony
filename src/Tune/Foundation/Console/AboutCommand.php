...
class AboutCommand extends Command
{
        ...
        collect(static::$data)
            ->map(fn ($items) => collect($items)
                ->map(function ($value) {
                    ...
                    if (is_string($value)) {
                        $value = $this->harmony->make($value);
                    }

                    return collect($this->harmony->call($value))
     ...
     *
     * @param  \Tune\Support\Collection  $data
     * @return void
     */
    ...
    protected function gatherApplicationInformation()
    {
        static::addToSection('Environment', fn () => [
            ...
            'Harmony Version' => $this->harmony->version(),
            'PHP Version' => phpversion(),
            'Composer Version' => $this->composer->getVersion() ?? '<fg=yellow;options=bold>-</>',
            'Environment' => $this->harmony->environment(),
            'Debug Mode' => config('app.debug') ? '<fg=yellow;options=bold>ENABLED</>' : 'OFF',
            'URL' => Str::of(config('app.url'))->replace(['http://', 'https://'], ''),
            'Maintenance Mode' => $this->harmony->isDownForMaintenance() ? '<fg=yellow;options=bold>ENABLED</>' : 'OFF',
        ]);

        static::addToSection('Cache', fn () => [
            'Config' => $this->harmony->configurationIsCached() ? '<fg=green;options=bold>CACHED</>' : '<fg=yellow;options=bold>NOT CACHED</>',
            'Events' => $this->harmony->eventsAreCached() ? '<fg=green;options=bold>CACHED</>' : '<fg=yellow;options=bold>NOT CACHED</>',
            'Routes' => $this->harmony->routesAreCached() ? '<fg=green;options=bold>CACHED</>' : '<fg=yellow;options=bold>NOT CACHED</>',
            'Views' => $this->hasPhpFiles($this->harmony->storagePath('framework/views')) ? '<fg=green;options=bold>CACHED</>' : '<fg=yellow;options=bold>NOT CACHED</>',
        ]);
