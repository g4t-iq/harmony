...
use Tune\Http\Client\Events\ConnectionFailed;
use Tune\Http\Client\Events\RequestSending;
use Tune\Http\Client\Events\ResponseReceived;
...
class PendingRequest
{
     ...
     *
     * @var \Tune\Http\Client\Factory|null
     */
    protected $factory;
     ...
     *
     * @var \Tune\Support\Collection|null
     */
     ...
     *
     * @var \Tune\Http\Client\Request|null
     */
    protected $request;
     ...
     * @return \Tune\Http\Client\Response
     */
     ...
     *
     * @param  callable  $callback
     * @return array<array-key, \Tune\Http\Client\Response>
     */
    ...
    protected function sendRequest(string $method, string $url, array $options = [])
    {
        ...
        $harmonyData = $this->parseRequestData($method, $url, $options);
    ...
    protected function parseRequestData($method, $url, array $options)
    {
        ...
        $harmonyData = $options[$this->bodyFormat] ?? $options['query'] ?? [];

        $urlString = Str::of($url);

        if (empty($harmonyData) && $method === 'GET' && $urlString->contains('?')) {
            $harmonyData = (string) $urlString->after('?');
        }

        if (is_string($harmonyData)) {
            parse_str($harmonyData, $parsedData);

            $harmonyData = is_array($parsedData) ? $parsedData : [];
        }

        if ($harmonyData instanceof JsonSerializable) {
            $harmonyData = $harmonyData->jsonSerialize();
        }

        return is_array($harmonyData) ? $harmonyData : [];
    }
            ...
            return function ($request, $options) use ($handler) {
                ...
                return $promise->then(function ($response) use ($request, $options) {
                    $this->factory?->recordRequestResponsePair(
                        (new Request($request))->withData($options['harmony_data']),
        ...
        return function ($handler) {
            return function ($request, $options) use ($handler) {
                $response = ($this->stubCallbacks ?? collect())
                     ->map
                     ->__invoke((new Request($request))->withData($options['harmony_data']), $options)
                     ->filter()
                     ->first();
        ...
        return tap($request, function (&$request) use ($options) {
            $this->beforeSendingCallbacks->each(function ($callback) use (&$request, $options) {
                $callbackResult = call_user_func(
                    $callback, (new Request($request))->withData($options['harmony_data']), $options, $this
                );
