...
use Tune\Queue\Events\JobRetryRequested;
...
class RetryCommand extends Command
{
    ...
    public function handle()
    {
        ...
        foreach ($ids as $id) {
            $job = $this->harmony['queue.failer']->find($id);

            if (is_null($job)) {
                $this->components->error("Unable to find failed job with ID [{$id}].");
            } else {
                $this->harmony['events']->dispatch(new JobRetryRequested($job));

                $this->components->task($id, fn () => $this->retryJob($job));

                $this->harmony['queue.failer']->forget($id);
            }
        }
    ...
    protected function getJobIds()
    {
        ...
        if (count($ids) === 1 && $ids[0] === 'all') {
            return Arr::pluck($this->harmony['queue.failer']->all(), 'id');
        }
    ...
    {
        $ids = collect($this->harmony['queue.failer']->all())
                        ->where('queue', $queue)
                        ->pluck('id')
                        ->toArray();
    ...
    {
        $this->harmony['queue']->connection($job->connection)->pushRaw(
    ...
    protected function refreshRetryUntil($payload)
    {
        ...
        } elseif ($this->harmony->bound(Encrypter::class)) {
            $instance = unserialize($this->harmony->make(Encrypter::class)->decrypt($payload['data']['command']));
        }
