...
use Tune\Contracts\Queue\Job;
use Tune\Queue\Events\JobFailed;
...
class WorkCommand extends Command
{
     ...
     *
     * @param  \Tune\Queue\Worker  $worker
    ...
    public function handle()
    {
        ...
        $connection = $this->argument('connection')
                        ?: $this->harmony['config']['queue.default'];
     ...
     *
     * @return \Tune\Queue\WorkerOptions
     */
    ...
    {
        $this->harmony['events']->listen(JobProcessing::class, function ($event) {
            $this->writeOutput($event->job, 'starting');
        });

        $this->harmony['events']->listen(JobProcessed::class, function ($event) {
            $this->writeOutput($event->job, 'success');
        });

        $this->harmony['events']->listen(JobReleasedAfterException::class, function ($event) {
            $this->writeOutput($event->job, 'released_after_exception');
        });

        $this->harmony['events']->listen(JobFailed::class, function ($event) {
    ...
    {
        $queueTimezone = $this->harmony['config']->get('queue.output_timezone');

        if ($queueTimezone &&
            $queueTimezone !== $this->harmony['config']->get('app.timezone')) {
     ...
     *
     * @param  \Tune\Queue\Events\JobFailed  $event
     * @return void
     */
    protected function logFailedJob(JobFailed $event)
    {
        $this->harmony['queue.failer']->log(
    ...
    {
        return $this->option('queue') ?: $this->harmony['config']->get(
    ...
    {
        return $this->option('force') ? false : $this->harmony->isDownForMaintenance();
    }
}
