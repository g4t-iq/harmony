...
use Tune\View\Compilers\ComponentTagCompiler;
...
class DynamicComponent extends Component
{
     ...
     *
     * @var \Tune\View\Compilers\BladeTagCompiler
     */
     ...
     *
     * @return \Tune\Contracts\View\View|string
     */
    public function render()
    {
        $template = <<<'EOF'
<?php extract(collect($attributes->getAttributes())->mapWithKeys(function ($value, $key) { return [Tune\Support\Str::camel(str_replace([':', '.'], ' ', $key)) => $value]; })->all(), EXTR_SKIP); ?>
{{ props }}
...
EOF;
        ...
        return function ($data) use ($template) {
            ...
            return str_replace(
                ...
                [
                    ...
                    $this->compileSlots($data['__harmony_slots']),
                    '{{ $slot ?? "" }}',
                ],
                $template
            );
     ...
     *
     * @return \Tune\View\Compilers\ComponentTagCompiler
     */
