...
use Tune\View\AnonymousComponent;
use Tune\View\DynamicComponent;
use Tune\View\ViewFinderInterface;
use InvalidArgumentException;
use ReflectionClass;

/**
 * @author Spatie bvba <info@spatie.be>
 * @author Taylor Otwell <taylor@harmony.com>
 */
class ComponentTagCompiler
{
     ...
     *
     * @var \Tune\View\Compilers\BladeCompiler
     */
    protected $blade;
     ...
     * @param  \Tune\View\Compilers\BladeCompiler|null  $blade
     * @return void
     */
    ...
    protected function componentString(string $component, array $attributes)
    {
        ...
        if (! class_exists($class)) {
            $view = Str::startsWith($component, 'mail::')
                ? "\$__env->getContainer()->make(Tune\\View\\Factory::class)->make('{$component}')"
                : "'$class'";
...
<?php if (isset($attributes) && $attributes instanceof Tune\View\ComponentAttributeBag && $constructor = (new ReflectionClass('.$class.'::class))->getConstructor()): ?>
...
<?php $component->withAttributes(['.$this->attributesToString($attributes->all(), $escapeAttributes = $class !== DynamicComponent::class).']); ?>';
     ...
     *
     * @param  \Tune\Contracts\View\Factory  $viewFactory
     * @param  string  $component
     * @return string|null
     */
        ...
        return preg_replace_callback(
            '/@(class)(\( ( (?>[^()]+) | (?2) )* \))/x', function ($match) {
                if ($match[1] === 'class') {
                    ...
                    return ":class=\"\Tune\Support\Arr::toCssClasses{$match[2]}\"";
                }
        ...
        return preg_replace_callback(
            '/@(style)(\( ( (?>[^()]+) | (?2) )* \))/x', function ($match) {
                if ($match[1] === 'style') {
                    ...
                    return ":style=\"\Tune\Support\Arr::toCssStyles{$match[2]}\"";
                }
        ...
        return collect($attributes)
                ->map(function (string $value, string $attribute) use ($escapeBound) {
                    return $escapeBound && isset($this->boundAttributes[$attribute]) && $value !== 'true' && ! is_numeric($value)
                                ? "'{$attribute}' => \Tune\View\Compilers\BladeCompiler::sanitizeComponentAttribute({$value})"
                                : "'{$attribute}' => {$value}";
                })
                ->implode(',');
    }
