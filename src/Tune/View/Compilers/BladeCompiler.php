...
class BladeCompiler extends Compiler implements CompilerInterface
{
     ...
     *
     * @param  \Tune\View\Component  $component
     * @return string
     */
    ...
    public function if($name, callable $callback)
    {
        ...
        $this->directive($name, function ($expression) use ($name) {
            return $expression !== ''
                    ? "<?php if (\Tune\Support\Facades\Blade::check('{$name}', {$expression})): ?>"
                    : "<?php if (\Tune\Support\Facades\Blade::check('{$name}')): ?>";
        });
        ...
        $this->directive('else'.$name, function ($expression) use ($name) {
            return $expression !== ''
                ? "<?php elseif (\Tune\Support\Facades\Blade::check('{$name}', {$expression})): ?>"
                : "<?php elseif (\Tune\Support\Facades\Blade::check('{$name}')): ?>";
        });
    ...
    public function aliasInclude($path, $alias = null)
    {
        ...
        $this->directive($alias, function ($expression) use ($path) {
            ...
            return "<?php echo \$__env->make('{$path}', {$expression}, \Tune\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>";
        });
    }
