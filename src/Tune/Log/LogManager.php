...
class LogManager implements LoggerInterface
{
     ...
     *
     * @param  string  $name
     * @param  \Tune\Log\Logger  $logger
     * @return \Tune\Log\Logger
     */
    ...
    protected function createEmergencyLogger()
    {
        ...
        $handler = new StreamHandler(
            $config['path'] ?? $this->app->storagePath().'/logs/harmony.log',
            $this->level(['level' => 'debug'])
        );

        return new Logger(
            new Monolog('harmony', $this->prepareHandlers([$handler])),
            $this->app['events']
        );
    }
